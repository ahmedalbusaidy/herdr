...
| --- | --- | --- |
| ライフサイクル権威 | Pi、OMP、Kimi Code CLI、OpenCode、Kilo Code CLI、MastraCode、Reasonix | インストールされ、そのペインについて能動的に報告している間は、フックまたはプラグインのイベントが `idle`、`working`、`blocked` を決定します。同じライフサイクル権威に対して、Herdr はスクリーンマニフェストのフォールバックを併用しません。 |
...
Reasonix のフックをインストールします:

```bash
herdr integration install reasonix
```

フックは Reasonix が Herdr ペイン内で実行されている間、セッション ID とライフサイクル状態 (working、idle、blocked) を Herdr に報告します。Herdr はデフォルトで `~/.reasonix` を使用し、`REASONIX_HOME` が設定されている場合はそちらを使用します。インストールは `hooks/herdr-agent-state.sh` を書き込み、`settings.json` にライフサイクルフックのエントリを登録します。アンインストールは Herdr が所有するフックファイルとそのエントリを削除します。

Reasonix がセッション開始イベントを発行した後、Herdr は報告されたセッション id を使って `reasonix --resume <id>` でペインを resume できます。

Reasonix は Unix (macOS/Linux) のみを提供します。
