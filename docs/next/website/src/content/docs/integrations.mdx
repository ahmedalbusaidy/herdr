...
herdr integration install reasonix
```
...
herdr integration uninstall reasonix
```
...
| --- | --- | --- |
| Lifecycle authority | Pi, OMP, Kimi Code CLI, OpenCode, Kilo Code CLI, MastraCode, Reasonix | When installed and actively reporting for the pane, hook or plugin events author `idle`, `working`, and `blocked`. Herdr does not also use screen manifest fallback for that same lifecycle authority. |
...
Install the Reasonix hook:

```bash
herdr integration install reasonix
```

The hook reports session identity as well as full lifecycle state (working, idle, blocked) to Herdr while Reasonix runs inside a Herdr pane. Reasonix is a full lifecycle authority: its hook authors the visible idle/working/blocked state directly.

Herdr uses `~/.reasonix` by default, or `REASONIX_HOME` when set. The Reasonix config directory must already exist. Install writes `hooks/herdr-agent-state.sh` under the Reasonix config directory and registers the lifecycle hook entries in `settings.json`, preserving any entries you have written yourself. Uninstall removes exactly the Herdr-owned hook file and its `settings.json` entries.

After Reasonix emits a session start event, Herdr can use the reported session id to resume the pane with `reasonix --resume <id>`. The installed hook is managed by Herdr and is rewritten on reinstall, so add your own hooks as separate `settings.json` entries instead of editing it. You can confirm what Reasonix has loaded with `reasonix hook list --json` or the in-session `/hooks` command.

Reasonix ships a Unix (macOS/Linux) binary only and has no Windows build, so this integration provides the POSIX `herdr-agent-state.sh` hook and no `.ps1` counterpart.
