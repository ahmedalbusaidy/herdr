...
| 集成类型 | 智能体 | 效果 |
| --- | --- | --- |
| 生命周期权威 | Pi、OMP、Kimi Code CLI、OpenCode、Kilo Code CLI、MastraCode、Reasonix | 已安装且在为该窗格主动上报时,由钩子或插件事件决定 `idle`、`working` 和 `blocked`。对同一个生命周期权威,Herdr 不再使用屏幕清单兜底。 |
...
当 Reasonix 在 Herdr 窗格中运行时，该钩子会向 Herdr 报告会话身份以及生命周期状态（working、idle、blocked）。Herdr 默认使用 `~/.reasonix`，若设置了 `REASONIX_HOME` 则使用该目录。安装会写入 `hooks/herdr-agent-state.sh` 并在 `settings.json` 中注册生命周期钩子条目。卸载会删除 Herdr 拥有的钩子文件及相应条目。

Reasonix 发出会话启动事件后，Herdr 可以用上报的会话 id 通过 `reasonix --resume <id>` 恢复窗格。

Reasonix 仅提供 Unix（macOS/Linux）版本。
